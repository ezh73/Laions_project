...
export default function Dashboard({ user }) {
    ...
    useEffect(() => {
        const smartFetchData = async () => {
            ...
            setError('');
            try {
                // 1. health API로 시즌 모드와 관리자 모드를 먼저 확인합니다.
                const healthRes = await getHealth();
                const currentMode = healthRes.data.season_mode;
                const currentAdminMode = healthRes.data.admin_mode; // 💡 관리자 모드 값 가져오기
                
                setSeasonMode(currentMode);
                setIsAdminMode(currentAdminMode); // 💡 관리자 모드 상태 설정

                // 2. 확인된 시즌 모드에 따라 올바른 API를 호출합니다.
                let response;
    ...
    return (
        <Box sx={{ display: 'flex', flexDirection: { xs: 'column', md: 'row' }, gap: 3 }}>
            ...
            <Box sx={{ display: 'flex', flexDirection: 'column', flex: '1', gap: 3 }}>
                {/* 💡 AiPerformanceCard에 isAdminMode와 seasonMode 상태를 props로 전달합니다. */}
